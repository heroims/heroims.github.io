...
SSB 蓝牙模块底层依赖于 BLE（低功耗蓝牙）实现设备间的发现与数据传输。理解 BLE 在 Android 和 iOS 之间的互通机制，对理解 SSB 蓝牙模块的通信方式很有帮助。

#### 平台支持演进

蓝牙的互相发现并不难实现，无论 Android 还是 iOS 都有现成的 API 提供，难点在于数据传输。传统经典蓝牙在 iOS 上受限严重——苹果的蓝牙拒绝接入不信任的设备。

- **iOS 6.0** 开始支持 BLE 低功耗蓝牙
- **Android 4.3** 开始支持 BLE
- 但此阶段仅支持 Android 作为主机（Central）连接 iOS 从机（Peripheral），反之不行
- **Android 5.0** 谷歌正式支持主从机的灵活建立，才真正奠定了 Android 和 iOS 的 BLE 互通基础

#### 角色与通信模型

BLE 蓝牙的角色分为：**广播者（Advertiser）**、**扫描者（Scanner）**、**从设备（Peripheral/Slave）**、**主设备（Central/Master）**、**发起者（Initiator）**。主设备由发起者、扫描者转化而来，从设备则由广播者转化而来。

扫描阶段需要注意平台差异：iOS 扫描到的设备 ID 是 UUID，而 Android 扫描到的是 MAC 地址。iOS 每次开关蓝牙后 UUID 可能变化，因此需要每次使用最新的扫描结果。

**基本通信流程**：

1. 主机扫描指定服务 ID 的设备
2. 连接设备后找到对应服务下的特征值（Characteristic）进行订阅
3. 写入操作需要使用**带回执的写入**（Write with Response），确保每条消息确认收到后再发下一条
4. 从机收到写入请求后给出回执，将消息投递到业务层
5. 从机如果要给主机发消息，只需改变自己服务中的特征值，主机通过订阅更新即可收到
...
```
原始数据 → 分包（每包 18 字节数据 + 2 字节标识）
         → 收端根据标识合并还原
```

#### 连接数限制

BLE 最多支持 **7 个设备同时连接**。从机在发现第 7 个设备连接时，需要主动踢掉旧连接，防止影响正在通信的其他设备。
